...
export default defineTarget<Config>({
  ...
  async build(ctx, config) {
    ...
    try {
      ...
      await exec('npm', ['install', '-g', '@paypal/cli'], {
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        'PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET required. Run: sh1pt secret set PAYPAL_CLIENT_ID <id> && sh1pt secret set PAYPAL_CLIENT_SECRET <secret>'
      );
    }

    // 3. Configure CLI with credentials
    try {
      await exec('paypal', ['config', 'set', 'client-id', clientId], {
        log: ctx.log, throwOnNonZero: true,
      });
      await exec('paypal', ['config', 'set', 'client-secret', clientSecret], {
        log: ctx.log, throwOnNonZero: true,
      });
